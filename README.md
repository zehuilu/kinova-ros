...
#--------------------------------------------------------------------
# Read this part after you install this repository.

The demos are all with this specific arm type: j2s7s300. Demo 1 and Demo 2 have two different types of control frameworks. In summary, if you launch Demo 1, you can only send single joint command (joint angle) to each joint. You won't be able to make the arm execute a trajectory of joint angles. As for Demo 2, you can control each joint by sending trajectory commands. In this simulation, the trajectory command at least should include the desired final joint state. You can also specifiy trajectory waypoints in some time steps within the total time duration. But some robots hardware or simulations may require a specific time frequency for the trajectory command.
...
Open the first terminal, and launch the Gazebo simulator
```
...
## Installation
You need to install the full ROS first. [Here](http://wiki.ros.org/melodic/Installation/Ubuntu) is a tutorial about how to install it on Ubuntu 18.04. The installation for other Linux systems should be similar.
